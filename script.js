...
const weaponsData = {
    "돌격 소총": [
        ...
        },
        {
            ...
            "manufacturerLogo": "assets/fn-logo.png",
    ...
    "기관단총": [
        ...
        },
        {
            ...
            "image": "",
            "images": [],
            "description": "Super V 반동 억제 메커니즘을 통해 경이적인 1200 RPM 연사 속도에서도 완벽한 총구 제어를 실현한 기관단총입니다.",
            "stats": {
                "recoil": "55",
                "sway": "95",
                "ergonomics": "76",
...
function restoreAppState() {
    ...
    try {
        ...
        if (state.category && state.itemId) {
            ...
            if (state.category === 'all') {
                for (const cat of Object.keys(dataSource)) {
                    ...
                    if (it) {
                        expandCategoryAndSelectItem(state.panel, cat, it);
                        if (state.panel === 'gear') {
                            showGearDetail(it, 'all', state.galleryIndex || 0);
                        } else if (state.panel === 'attachment') {
                            showAttachmentDetail(it, 'all', state.galleryIndex || 0);
                        } else {
                            showWeaponDetail(it, 'all', state.galleryIndex || 0);
                        }
                        return true;
                    }
                }
                ...
                if (it) {
                    expandCategoryAndSelectItem(state.panel, state.category, it);
                    if (state.panel === 'gear') {
                        showGearDetail(it, state.category, state.galleryIndex || 0);
                    } else if (state.panel === 'attachment') {
                        showAttachmentDetail(it, state.category, state.galleryIndex || 0);
                    } else {
                        showWeaponDetail(it, state.category, state.galleryIndex || 0);
                    }
                    return true;
                }
            }
...
function expandCategoryAndSelectItem(panelType, categoryKey, item) {
    const panelId = panelType === 'gear' ? 'gearPanel' : (panelType === 'attachment' ? 'attachmentPanel' : 'weaponPanel');
    const panel = document.getElementById(panelId);
    if (!panel) return;
    
    panel.querySelectorAll('.category-item').forEach(catItem => {
        const link = catItem.querySelector('.category-link');
        const list = catItem.querySelector('.weapon-list-sidebar');
        if (link && link.dataset.category === categoryKey) {
            link.classList.add('active');
            if (list) {
                list.style.display = 'block';
                if (panelType === 'gear') {
                    renderGearInSidebar(categoryKey, list);
                } else if (panelType === 'attachment') {
                    renderAttachmentsInSidebar(categoryKey, list);
                } else {
                    renderWeaponsInSidebar(categoryKey, list);
                }
                list.querySelectorAll('.weapon-item-sidebar').forEach(wItem => {
                    if ((wItem.dataset.weaponId || wItem.dataset.itemId) === item.id) {
                        wItem.classList.add('active');
                    }
                });
            }
        }
...
function toggleCategory(categoryKey, itemListContainer, panelType) {
    ...
    if (!isExpanded) {
        ...
        // 아이템 목록 렌더링
        if (panelType === 'gear') {
            renderGearInSidebar(categoryKey, itemListContainer);
        } else if (panelType === 'attachment') {
            renderAttachmentsInSidebar(categoryKey, itemListContainer);
        } else {
            renderWeaponsInSidebar(categoryKey, itemListContainer);
        }
    }
}
...
// 사이드바에 기어 목록 렌더링
function renderGearInSidebar(categoryKey, container) {
    renderItemsInSidebar(categoryKey, container, gearData);
}

// 사이드바에 부착물 목록 렌더링
function renderAttachmentsInSidebar(categoryKey, container) {
    renderItemsInSidebar(categoryKey, container, attachmentData);
}

// 사이드바에 아이템 목록 렌더링 (무기/기어/부착물 공통)
function renderItemsInSidebar(categoryKey, container, dataSource) {
    ...
    const isAttachment = dataSource === attachmentData;
    ...
    sortedItems.forEach(item => {
        ...
        li.dataset.panelType = isGear ? 'gear' : (isAttachment ? 'attachment' : 'weapon');
        ...
        li.addEventListener('click', (e) => {
            ...
            if (isGear) {
                ...
                showAttachmentDetail(item, categoryKey);
            } else {
...
function searchItems(query) {
    if (!query || query.trim() === '') {
        if (currentPanel === 'gear') {
            renderGearCategories();
        } else if (currentPanel === 'attachment') {
            renderAttachmentCategories();
        } else {
            renderCategories();
        }
    ...
    const currentData = currentPanel === 'gear' ? gearData : (currentPanel === 'attachment' ? attachmentData : weaponsData);
    const categoryListId = currentPanel === 'gear' ? 'gearCategoryList' : (currentPanel === 'attachment' ? 'attachmentCategoryList' : 'categoryList');
    ...
    Object.keys(currentData).forEach(categoryKey => {
        const matchingItems = currentData[categoryKey].filter(item => {
            ...
            return nameMatch || keywordMatch || manufacturerMatch;
        });

        if (matchingItems.length > 0) {
            const categoryItem = createCategoryItem(categoryKey, matchingItems.length, categoryKey, currentPanel);
            const itemList = categoryItem.querySelector('.weapon-list-sidebar');
            if (itemList) {
                itemList.style.display = 'block';
                itemList.innerHTML = '';
                matchingItems.forEach(item => {
                    const itemEl = document.createElement('div');
                    itemEl.className = 'weapon-item-sidebar';
                    itemEl.dataset.itemId = item.id;
                    itemEl.dataset.weaponId = item.id;
                    
                    const nameSpan = document.createElement('span');
                    nameSpan.className = 'weapon-name-sidebar';
